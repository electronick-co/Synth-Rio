...
#error "Unsupported AUTH Negotiation"
    ...
    // Sim800 Power key set
    uint8_t power_key_gpio = 4;
    uint8_t led_gpio = 13;
    uint8_t sim800_PWR_en_gpio = 23;

    gpio_pad_select_gpio(sim800_PWR_en_gpio);
    gpio_set_direction(sim800_PWR_en_gpio, GPIO_MODE_OUTPUT);
    gpio_pad_select_gpio(power_key_gpio);
    gpio_set_direction(power_key_gpio, GPIO_MODE_OUTPUT);
    gpio_pad_select_gpio(led_gpio);
    gpio_set_direction(led_gpio, GPIO_MODE_OUTPUT);
    ESP_LOGI(TAG, "GPIO Setup");

    ESP_LOGI(TAG, "Modem Reset");
    gpio_set_level(sim800_PWR_en_gpio,0);
    vTaskDelay(200 / portTICK_RATE_MS);
    gpio_set_level(sim800_PWR_en_gpio,1);
    vTaskDelay(2000 / portTICK_RATE_MS);

    ESP_LOGI(TAG, "Modem PowerKey enable");
    gpio_set_level(led_gpio,1);
    gpio_set_level(power_key_gpio,1);
    vTaskDelay(2000 / portTICK_RATE_MS);
    gpio_set_level(power_key_gpio,0);
